...
<script lang="ts">
    ...
    type Topic = 'Algebra' | 'Calculus' | 'Geometry' | 'Trigonometry';
    ...
    const topics: Topic[] = ['Algebra', 'Calculus', 'Geometry', 'Trigonometry'];
    ...
    const problemTemplates = {
        ...
        Calculus: {
            Basic: [
                "Find the derivative of: {a}x² + {b}x + {c}",
                "Find the integral of: {a}x + {b}",
            ],
            Intermediate: [
                "Find the derivative of: {a}sin({b}x) + {c}cos({d}x)",
                "Find the integral of: {a}x² + {b}x + {c}",
            ],
            Advanced: [
                "Find the limit as x approaches infinity: ({a}x² + {b}x) / ({c}x² + {d})",
                "Find the derivative of: {a}ln(x) + {b}e^({c}x)",
            ]
        },
    ...
    function generateRandomProblem(): Problem {
        ...
        const templates = problemTemplates[topic as keyof typeof problemTemplates]?.[difficulty] || ["Solve: x + 1 = 0"];
    ...
    async function checkSolutionWithGemini(imageBase64: string, problem: Problem) {
        if (!GEMINI_API_KEY) {
            return {
                correct: false,
                feedback: 'API key not configured. Please add VITE_GEMINI_API_KEY to your .env file.'
            };
        }
        ...
        try {
            ...
            return {
                correct: false,
                feedback: `Error checking solution: ${error instanceof Error ? error.message : 'Unknown error'}`
            };
        }
    }
        ...
        <Card class="p-6">
            ...
            <div class="space-y-6">
                <div class="p-4 bg-gray-50 rounded-lg">
                    ...
                    <p class="whitespace-pre-line">{currentProblem.question}</p>
                </div>
